...
- WIF は `crawler-image.yml` と GitHub Environment `dev` に限定する。branch は条件に入れない。prod は同じ pool `github` を使い、IAM は `attribute.environment` で分ける。別 GitHub リポジトリは別 pool にする
