...
5. `devgist-github`（ローカルのみ）
   - GitHub Environment / repository variable / repository secret を書く。variable の値は `ops` の outputs を `terraform_remote_state` で参照する。secret の値は gitignore 済み `secrets.auto.tfvars`
