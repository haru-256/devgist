...
- Cursor Cloud 用 WIF（pool `cursor` / provider `oidc`）は `devgist-ops` で定義する。datalake への `objectViewer` / `objectCreator` も `devgist-ops` が federated principal に直接付与する（[INFRA-ADR-015](../../docs/adr/infra/015-cursor-wif-iam-in-ops.md)）。apply 順は data → ops → app。`cursor_wif_audience` は後続の OIDC mint で使う。credential config に SA impersonation は入れない。
