...
- DevGist の GCP プロジェクト構成として、Data Project と App Project を環境ごとに分離する構成を採用する。
- 初期フェーズでは `data` と `app` を分けた 2 プロジェクト構成を Dev / Prod で持ち、Terraform State 管理用に Mgmt Project を別途設ける。
- 将来、チーム構成や運用要件が変化した場合は、App Project のさらなる分割を再検討する。
