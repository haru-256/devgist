...
本 ADR は、カンファレンス論文クロールという**特定の例外運用**に対して、Scheduler 前提を絞り込み・精緻化する。カンファレンス論文クロールは手動実行やイベント駆動を主とし、実行時に `CONFERENCE_NAMES` と `YEARS` をオーバーライドして運用する。これは以前の ADR の方針を否定するものではないが、カンファレンスクロールの運用想定を明確に変更・特化させるものである。
