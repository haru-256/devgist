...
- `crawler_image` は variable を維持し、値は committed tfvars に置く。image の更新は「build（crawler-deploy が main で push）→ digest を tfvars に書き換える infra PR → merge で apply」の 2 PR 運用とする。digest PR の作成は [INFRA-ADR-021](./021-crawler-image-digest-pr.md)
...
- [[INFRA-ADR-021] crawler image digest の置換 PR を crawler-deploy が作成する](./021-crawler-image-digest-pr.md)
