...
- GitHub provider は `devgist-ops` に置く。owner と repository 名は複数箇所で使うので local にする。repository id と owner id は WIF condition にベタ書きする。Cursor OIDC の `repo_url` は owner と name から組み立てる。token は apply 時の環境変数 `GITHUB_TOKEN` とし、tfvars と state には書かない
