...
- ワークロード実行用 Service Account は dev / prod で別に設ける。Artifact Registry 権限は同一だが、SA を分けることで Cloud SQL・Secret Manager・GCS など周辺リソースへのアクセスを環境ごとに分離する。この方針は Cloud Run に限らず、GKE・Cloud Batch・Cloud Functions など DevGist が採用するすべての compute platform に適用する。
...
| 選択肢 | 向いている用途 | メリット | デメリット | 今回の評価 |
|---|---|---|---|---|
| Option A: dev / prod で SA を共有 | 管理対象を最小にしたい場合 | SA 数が少ない | dev SA が prod DB や prod secrets にアクセスできてしまう | 非採用 |
| Option B: dev / prod で SA を分離 | 周辺リソースへのアクセスを環境ごとに制御したい場合 | Cloud SQL・Secret Manager・GCS の権限を環境ごとに閉じられる。compute platform（Cloud Run / GKE / Cloud Batch 等）が変わっても同じ原則を適用できる | SA 数が増える | 採用 |
...
Artifact Registry リポジトリはアプリケーション単位で 1 本とし、dev / prod 共用・SHA 固定を採用する。ワークロード実行用 SA は compute platform の種類によらず環境ごとに分離する。
...
- ワークロード実行用 SA は dev / prod で別に作成する
- この方針は compute platform の種類（Cloud Run / Cloud Run Jobs / GKE / Cloud Batch / Cloud Functions 等）によらず統一して適用する
- Artifact Registry への read 権限（`roles/artifactregistry.reader`）は dev / prod SA ともに同じ
- SA を分ける目的は Artifact Registry ではなく、Cloud SQL・Secret Manager・GCS など周辺リソースへのアクセスを環境ごとに制御するため
...
ワークロード SA（devgist-app-dev）← Cloud Run / GKE / Cloud Batch 等すべてで共通
├── roles/artifactregistry.reader → crawler 等の pull 権限
├── dev Cloud SQL への接続権限
└── dev Secret Manager への read 権限

ワークロード SA（devgist-app-prod）← 同上
├── roles/artifactregistry.reader → crawler 等の pull 権限
├── prod Cloud SQL への接続権限
└── prod Secret Manager への read 権限
```
...
- SA のアプリケーション単位分離: 現設計は環境単位（dev / prod）の SA 分離にとどまる。アプリケーション数が増え、crawler・api・frontend 間で参照するリソースが大きく異なってきた場合は、アプリケーション単位（例: `crawler-dev-sa`, `api-dev-sa`）への細分化を再検討する

## Next Steps

1. `devgist-ops` の Terraform に Artifact Registry リポジトリ（`crawler` 等）を定義する
2. `devgist-app-dev` / `devgist-app-prod` の Terraform にワークロード用 SA を定義し、IAM を設定する
