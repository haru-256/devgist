...
- 初期フェーズでは全リポジトリへの push 権限を一括付与する（CI/CD 整備時にリポジトリ単位への制限を検討する）
...
- Artifact Registry への read 権限は、最小権限の原則に従い **リポジトリ単位**で付与する（プロジェクト全体への `roles/artifactregistry.reader` ではなく、各 SA が使うリポジトリのみ）
- SA を細分化する目的は最小権限の原則の徹底。あるアプリが侵害されても、他アプリの Cloud SQL・Secret Manager・GCS にアクセスできない
- Terraform で管理することで SA 増加のコストを抑える

### 初期構成

> 以下の名称は GCP project ID から `haru256-` プレフィックスを省略した表記（例: `devgist-ops` = GCP project `haru256-devgist-ops`）。
> Terraform environment のディレクトリパスとは異なる（例: `devgist-app-dev` の Terraform env は `environments/devgist-app/dev`）。
...
└── artifactregistry.writer → 全リポジトリへの push 権限（初期フェーズ。CI/CD 整備時に単位を絞る）

crawler-dev-sa（devgist-app-dev）← Cloud Run Jobs / GKE / Cloud Batch 等共通
├── artifactregistry.reader → crawler リポジトリのみの pull 権限（リポジトリ単位）
